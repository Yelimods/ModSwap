...
by Yelibam at www.fs-uk.com - Copyright © 2015 Yelibam - yelimods@mail.com

ModSwap v0.6 beta Manual
========================

Beta Release
------------
ModSwap is still in the beta stage, and is being released as such.  Further changes and additions are anticipated.  This manual will also be updated and improved.  Thank you for trying ModSwap.

About ModSwap
-------------
ModSwap is not an in-game mod.  It is a program to assist in file management for the in-game mods that you already have.  ModSwap is designed to work with Farming Simulator 15 and 2013.  Because of the similar file structure it should also work with versions 2011 and 2009, but this has not yet been tested.
The primary function of ModSwap is to enable you to create several groups of in-game mods (Mod Groups), then switch between these groups easily, according to which map or style you wish to play.  With so many embedded scripts in maps it is a good idea to avoid conflicts by keeping only one map and the relevant mods in the mods folder.  ModSwap enables you to create separate mods folders for up to 20 maps, and switch between them.  If you wish to test mods in a clean mods folder, or with different key mods such as Soil Mod, or MR, then ModSwap will enable you to do this with ease.
ModSwap is an ongoing project.  In the current version ModSwap consists primarily of three Windows Scripting files: Menu,  Builder and Switcher.  These are like the old style Windows Batch files, but have enhanced functions and commands.  More features will be added in the future.  ModSwap will not work with Mac computers, and will not work with Windows XP or earlier.
...
Part 1 Preparation
Firstly, please create a full backup copy of your entire Game Folder.  This is the folder called 'FarmingSimulator2013' or 'FarmingSimulator2015' and resides within 'My Documents\My Games\'.
Secondly, in the Game Folder, make a sub-folder called 'My Mods Backup' and make a backup copy of your default Mods Folder in that new sub-folder.
...
Part 2 Installation
Please note that the images in this document were made with ModSwap v0.5 beta.  There have been a few small changes in v0.6 beta, but the basic process is the same.
Place the ModSwap zip file in the Game Folder and extract all the files into the folder: ModSwap_v0.6 .  Select 'Extract to Folder', not 'Extract Here'.  Then check that the files are not in a nested sub-folder.
...
If you wish you can create a shortcut to ModSwapMenu.cmd on your desktop, to make it even quicker and easier to switch your Mod Groups.  To do this, right click on ModSwapMenu.cmd and select 'Send to' from the pop-up menu.  Then click on 'Desktop (create shortcut)'.
...
Running ModSwap
---------------
On first use you must run ModSwap Menu as it sets up other files for use by ModSwap.  You can run ModSwap Menu either from the shortcut on your desktop, or directly from the .cmd file.  Just double-click either one.  The on-screen instructions should be self-explanatory.
Log File
...
Later, if a log file already exists, Menu will ask if you want to continue the same log file, or start a new one.  From time to time you should start a new file so that it does not get too long.  If you choose to delete the Log File then it is actually archived in the ModSwap folder.  You can manually delete these files later if you wish.
...
* MR
* Cleared
Your original default mods folder has been backed up to #Mods_Old.  All your existing mods will be in here.
Note that my first choice of BritishIrish mods has been set as the default mods folder.
Be sure that you do not move, delete or rename these folders, or else ModSwap will not work.
Now you have the task of sorting out your existing mods into the appropriate Mod Group folder.  I find that there are a number of mods that I want in several, or all, Mod Groups.  There is no problem in having duplications across Mod Groups, although it will use a little more disk space.  You can then freely add or remove mods from these folders over time.
...
As with any program, please be sure that you don't move alter or rename any folders or files created by or used by ModSwap, as this will stop it from working correctly.
...
All these files are contained within the folder: ModSwap_v0.6.  This resides in your game folder, alongside the mods folder and savegame folders.
...
ModSwap including all accompanying files and intellectual concept is copyright © 2015 Yelibam - yelimods@mail.com
...
Yelibam at www.fs-uk.com - yelimods@mail.com | The ModSwap Project Manual